...
- Smart Pay Expo beta now also keeps recent device-local session/receipt snapshots for resume/history/recovery baseline UX, preserves locally available per-execution `sessionToken` access for resume/recover flows, can merge authenticated backend payment-history listing with local history, can fetch backend receipt snapshots for the active payment, now makes the authenticated-vs-device-token resume boundary explicit in UI/docs, surfaces route-progress/recovery-state hints directly inside the session history list, maps quoted route steps to linked-vs-pending proof coverage in the receipt view while keeping unmatched additional tx refs visible separately, and avoids reusing one observed tx ref across multiple quoted steps with the same role/network pair
