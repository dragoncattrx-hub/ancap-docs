...
- Smart Pay Expo beta now also keeps recent device-local session/receipt snapshots for resume/history/recovery baseline UX, can merge authenticated backend payment-history listing with local history, and can fetch backend receipt snapshots for the active payment
...
| Public `ancap-docs` split | In progress / repo prep hardened | The public docs repo now exists at `dragoncattrx-hub/ancap-docs`, but it is still empty and the seed bundle + repeatable export/bootstrap path are being prepared in-repo via `docs/ANCAP_DOCS_SPLIT.md`, `docs/ANCAP_DOCS_REPO_BOOTSTRAP.md`, `docs/ANCAP_DOCS_CONTRIBUTOR_INTAKE_SEED.md`, `docs/ANCAP_DOCS_LABEL_SEED.md`, `docs/ANCAP_DOCS_DISCUSSIONS_SEED.md`, `docs/ANCAP_DOCS_MILESTONE_SEED.md`, `docs/ANCAP_DOCS_PROJECT_BOARD_SEED.md`, `docs/ANCAP_DOCS_REPO_SETTINGS_SEED.md`, `docs/ANCAP_DOCS_UPDATE_CADENCE_SEED.md`, `docs/ANCAP_DOCS_CI_SEED.md`, `docs/ANCAP_DOCS_DEPENDABOT_SEED.md`, `.github/CODEOWNERS`, `.github/bootstrap/README.md`, `.github/bootstrap/ancap-docs-contributor-intake.json`, `.github/bootstrap/ancap-docs-labels.json`, `.github/bootstrap/ancap-docs-milestones.json`, `.github/bootstrap/ancap-docs-discussions.json`, `.github/bootstrap/ancap-docs-project-board.json`, `.github/bootstrap/ancap-docs-repo-settings.json`, `.github/bootstrap/ancap-docs-update-cadence.json`, `.github/bootstrap/ancap-docs-ci.json`, `.github/bootstrap/ancap-docs-ci-workflow.yml`, `.github/bootstrap/ancap-docs-dependabot.yml`, `.github/workflows/docs-ci.yml`, `scripts/export_ancap_docs.py`, and `scripts/bootstrap_ancap_docs_repo.py`; the export now ships a docs-focused root README, includes public-safe GitHub issue/PR templates plus a baseline CODEOWNERS review-routing seed, also carries a dedicated contributor-intake seed and matching machine-readable metadata so issue/PR lanes and security-report routing stay explicit outside the raw templates, carries the future repo bootstrap/settings/labels/Discussions/milestones/project-board/repo-settings/update-cadence/CI checklist plus reusable contributor-intake, label, Discussions, milestone, project-board, repo-settings, update-cadence, CI, and Dependabot seeds, also includes copy-ready pinned-topic/update-post templates, a bootstrap-seed README, machine-readable bootstrap metadata for bulk/scripting-friendly setup, a copy-ready Docs CI workflow with an explicit default `Docs CI / docs-bundle` required-check context, a docs-repo-specific Dependabot config/template so the future public repo does not inherit monorepo-only ecosystems by accident, the matching exported workflow template referenced by the CI seed, and a gh-driven helper for public repo creation, repo settings/labels/milestones, and the default-branch protection payload so launch setup does not depend on retyping seed values by hand once ownership/access and public CI status-check context names exist; the next honest external step is pushing the exported bundle before applying repo settings/labels/milestones/branch protection, and the exported Docs CI guard still enforces byte-for-byte alignment between the docs-repo Dependabot template and the shipped public `.github/dependabot.yml` inside the bundle while Backend CI reruns the export/public-trust regression slice when those bundle inputs change. |
